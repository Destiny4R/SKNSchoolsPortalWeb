...
namespace Bethel_Heritage_Portal_Web.Pages.vs
{
    [Authorize(Roles = SD.IsStudent)]
    public class junior_SheetModel : PageModel
    {
		...
		public junior_SheetModel(ApplicationDbContext dbContext,UserManager<ApplicationUser> userManager)
		{
		...
		public async Task<IActionResult> OnGet(int session, int classes, int subclass, string term)
		{
			if (ModelState.IsValid)
			{
                ...
                if (admin.Any())
                {
					...
					signature = settings.PrincipalSignature;
